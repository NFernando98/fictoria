...
export default function BrowsePage({ isDarkMode }: BrowsePageProps) {
  ...
  // Set the initial type filter based on the URL parameter
  useEffect(() => {
    if (type) {
      const formattedType = type.charAt(0).toUpperCase() + type.slice(1);
      setSelectedFilters(prev => ({ ...prev, Type: formattedType }));
    }
  }, [type]);
  ...
  const filteredResults = dummyResults.filter(result => {
    // Filter by type
    if (type && result.type.toLowerCase() !== type.toLowerCase()) {
      return false;
    }
              ...
              {filterOptions.map((filter) => (
                <div key={filter.label} className="relative min-w-[140px]">
                  <select
                    ...
                    disabled={filter.label === 'Type'} // Disable type selection since it's determined by the route
                  >
